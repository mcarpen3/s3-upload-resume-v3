...
Client.prototype.uploadFile = function(params) {
    ...
    function startMultipartUpload(multipartUploadSize) {
        doWithRetry(tryCreateMultipartUpload, self.s3RetryCount, self.s3RetryDelay, function(err, data) {
            ...
            queueNeededParts(uploaderEE.UploadId, multipartUploadSize);
        });
    }
    ...
    function startResumeMultipartUpload(currentUpload) {
        doWithRetry(tryResumeMultipartUpload, self.s3RetryCount, self.s3RetryDelay, function(err, data) {
            ...
            if (data.Parts.length > 0) {
                ...
                queueNeededParts(uploaderEE.UploadId, data.Parts[0].Size || self.multipartUploadSize);
            } else {
                queueNeededParts(uploaderEE.UploadId, self.multipartUploadSize);
            }
        });
    }
                                ...
                                self.s3.uploadPart(extend({}, s3Params), function(err, data) {
                                    ...
                                    pend.wait(function() {
                                        ...
                                        if (!compareMultipartETag(data.ETag, multipartETag)) {
                                            // console.log(`Hash: '${hash}', Etag: ${cleanETag(data.ETag)}`);
                                            errorOccurred = true;
                                            uploaderEE.progressAmount -= overallDelta;
