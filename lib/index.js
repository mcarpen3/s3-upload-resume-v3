...
Client.prototype.uploadFile = function(params){
  ...
  function startPuttingObject(){
    if(localFileStat.size >= self.multipartUploadThreshold){
      ...
      if(debug){ console.log('testMultiparUpload', multipartUploadSize); }
      testMultiparUpload(multipartUploadSize);
    } else {
      if(debug){ console.log('tryPuttingObject', localFileStat.size); }
  ...
  function testMultiparUpload(multipartUploadSize){
  ...
  function tryTestMultipartUpload(cb){
    ...
    self.s3Pend.go(function(pendCb){
      ...
      self.s3.listMultipartUploads({Bucket: s3Params.Bucket, Prefix: encodeSpecialCharacters(s3Params.Key)}, function(err, data){
        pendCb();
        if(fatalError) return;
        cb(err, data);
      });
  ...
  function tryStartResumeMultipartUpload(cb){
    ...
    self.s3Pend.go(function(pendCb){
      ...
      self.s3.listParts({Bucket: bucket, Key: upload.Key, UploadId: upload.UploadId}, function(err, data){
        pendCb();
        if(fatalError) return;
        cb(err, data);
      });
  ...
  function tryPuttingObject(cb){
    self.s3Pend.go(function(pendCb){
      ...
      let pend = new Pend();
      let bar = new progBar.ProgressBar({filename: s3Params.Key, size: localFileStat.size});
