...
function Client(options) {
    options = options || {};
    this.s3 = options.s3Client || new S3Client(options.s3Options);
    ...
    function uploadSlice(slice) {
        ...
        function tryDeletingObjects(cb) {
            self.s3Pend.go(function (pendCb) {
                ...
                //     cb(err, data);
                // });
                self.s3.send(new DeleteObjectsCommand(params), function (err, data) {
                    pendCb();
                    cb(err, data);
                })
            });
...
Client.prototype.uploadFile = function (params) {
    ...
    function testMultipartUpload(multipartUploadSize) {
        doWithRetry(tryTestMultipartUpload, self.s3RetryCount, self.s3RetryDelay, function (err, data) {
            console.log('doWithRetry tryTestMultipartUpload', err, data)
            if (fatalError) return;
            if (err) return handleError(err);

            if (!data['Uploads'] || data.Uploads.length === 0) {
                console.log('startMultipartUpload')
                startMultipartUpload(multipartUploadSize);
            } else {
                console.log('startResumeMultipartUpload')
                    ...
                    crypto
                        ...
                        .on('finish', function () {
                            ...
                            if (
                                ...
                                console.log(`-------------starting UploadPartCommand ${JSON.stringify(s3Params, null, 2)}`)
