...
function Client(options) {
    options = options || {};
    this.s3 = options.s3Client || new S3Client(new ConfigServiceClient(options.s3Options));
    ...
    function uploadSlice(slice) {
        ...
        function tryDeletingObjects(cb) {
            self.s3Pend.go(function (pendCb) {
                ...
                const deleteResults = self.s3.send(new DeleteObjectsCommand(params))
                deleteResults.then(r => {
                    pendCb();
                    cb(null, r);
                })
                .catch(reason => {
...
Client.prototype.uploadFile = function (params) {
    // console.log(`begin uploadFile ${JSON.stringify(params, null, 2)}`)
    const self = this;
    ...
    uploaderEE.s3Bucket = s3Params.Bucket;
    uploaderEE.s3Key = s3Params.Key;
    ...
    function handleAbort() {
        fatalError = true;
        if (uploaderEE.UploadId) {
            self.s3.send(new AbortMultipartUploadCommand({
                Bucket: uploaderEE.s3Bucket,
                Key: uploaderEE.s3Key,
                UploadId: uploaderEE.UploadId
            }), (err, data) => {
                if (err) {
    ...
    function startPuttingObject() {
        if (localFileStat.size >= self.multipartUploadThreshold) {
            ...
            uploaderEE.emit('uploading', 'putting');
    ...
    function testMultipartUpload(multipartUploadSize) {
        doWithRetry(tryTestMultipartUpload, self.s3RetryCount, self.s3RetryDelay, function (err, data) {
            ...
            if (!data['Uploads'] || data.Uploads.length === 0) {
                ...
                uploaderEE.UploadId = data.Uploads[0].UploadId;
    ...
    function tryTestMultipartUpload(cb) {
        ...
        self.s3Pend.go(function (pendCb) {
            ...
            self.s3.send(new ListMultipartUploadsCommand(
                {
                    Bucket: s3Params.Bucket,
                    Prefix: s3Params.Key
                }),
    ...
    function startMultipartUpload(multipartUploadSize) {
        doWithRetry(tryCreateMultipartUpload, self.s3RetryCount, self.s3RetryDelay, function (err, data) {
            ...
            uploaderEE.emit('uploading', 'starting');
            uploaderEE.UploadId = data.UploadId;
            s3Params = {
                Bucket: s3Params.Bucket,
            ...
            queueNeededParts(multipartUploadSize);
        });
    }
    ...
    function startResumeMultipartUpload(currentUpload) {
        doWithRetry(tryResumeMultipartUpload, self.s3RetryCount, self.s3RetryDelay, function (err, data) {
            ...
            if (err) return handleError(err);
            uploaderEE.emit('data', data);
            uploaderEE.emit('uploading', 'resuming');
            s3Params = {
                Bucket: s3Params.Bucket,
                Key: encodeSpecialCharacters(s3Params.Key),
                SSECustomerAlgorithm: s3Params.SSECustomerAlgorithm,
            ...
            if (data.Parts.length > 0) {
                for (const part of data.Parts) {
                    uploadedParts[part.PartNumber] = {};
                    uploadedParts[part.PartNumber].LastModified = part.LastModified;
                    uploadedParts[part.PartNumber].Etag = part.ETag;
                    uploadedParts[part.PartNumber].Size = part.Size;
                }
                queueNeededParts(data.Parts[0].Size >= self.multipartUploadSize ? data.Parts[0].Size : self.multipartUploadSize);
            } else {
                queueNeededParts(self.multipartUploadSize);
            }
        });
    }

    function queueNeededParts(multipartUploadSize) {
        let cursor = 0;
        ...
        while (cursor < localFileStat.size) {
            ...
            parts.push(part);
            pend.go(makeUploadPartFn(start, end, part));
        }
        pend.wait(function (err) {
            if (fatalError) return;
            if (err) return handleError(err);
    ...
    function makeUploadPartFn(start, end, part) {
        return function (cb) {
        ...
        function tryUploadPart(cb) {
            ...
            self.s3Pend.go(function (pendCb) {
                ...
                const md5Stream = localFileSlicer.createReadStream({ start: start, end: end });
                let errorOccurred = false;
                md5Stream.on('error', function (err) {
                ...
                md5Stream.pipe(
                    crypto
                        ...
                        .setEncoding('hex')
                        .on('finish', function () {
                            let hash = this.read(); // this.read() === hash of part. Compare to Uploaded Parts if any, and upload part to s3 if don't match

                            if (
                                uploadedParts[part.PartNumber] &&
                                uploadedParts[part.PartNumber].hasOwnProperty('Etag') &&
                                uploadedParts[part.PartNumber].Etag.replace(/\"/g, '') === hash
                            ) {
                                uploaderEE.progressAmount += end - start;
                                ...
                                s3Params.UploadId = uploaderEE.UploadId;
                                ...
                                self.s3.send(new UploadPartCommand(extend({}, s3Params)), function (err, data) {
                                    if (fatalError || errorOccurred) return;
                                    if (err) {
                                        errorOccurred = true;
                                        ...
                                        try {
                                            ...
                                            console.error(partThis.firstCallStack);
                                            console.error(cbError);
                                        }
                                        partThis.firstCallStack = new Error().stack;
                                        return;
                                    }
                                    try {
                                        pendCb();
    ...
    function tryCompleteMultipartUpload(cb) {
        ...
        self.s3Pend.go(function (pendCb) {
            ...
            self.s3.send(new CompleteMultipartUploadCommand(s3Params), function (err, data) {
                pendCb();
                if (fatalError) return;
                cb(err, data);
            });
    ...
    function tryCreateMultipartUpload(cb) {
        ...
        self.s3Pend.go(function (pendCb) {
            if (fatalError) return pendCb();
            self.s3.send(new CreateMultipartUploadCommand(s3Params), function (err, data) {
                pendCb();
                if (fatalError) return;
                cb(err, data);
            });
    ...
    function tryResumeMultipartUpload(cb) {
        ...
        function getAllParts() {
            self.s3Pend.go(function (pendCb) {
                ...
                self.s3.send(new ListPartsCommand(s3PartParams), function (err, data) {
                    pendCb();
                    if (fatalError) return;
                    if (err) return cb(err);

                    if (data.IsTruncated) {
                        s3PartParams.PartNumberMarker = data.NextPartNumberMarker;
    ...
    function tryPuttingObject(cb) {
        self.s3Pend.go(function (pendCb) {
            ...
            self.s3.send(new PutObjectCommand(s3Params), function (err, data) {
                pendCb();
                if (fatalError) return;
                if (err) {
                    cb(err);
...
Client.prototype.downloadFile = function (params) {
    ...
    function doTheDownload(cb) {
        ...
        let errorOccurred = false;
        const request = self.s3.send(new GetObjectCommand(s3Params));
        request.then(res => {
            if (res.ContentLength === undefined) {
                ...
                const contentLength = parseInt(res.ContentLength, 10);
                downloaderEE.progressTotal = contentLength;
                downloaderEE.progressAmount = 0;
                downloaderEE.emit('progress');
                downloaderEE.emit('httpHeaders', res.ContentType);
                const eTag = cleanETag(res.ETag);
                const eTagCount = res.TagCount;

                const outStream = fs.createWriteStream(downloaderEE.localFile);
                const multipartETag = new MultipartETag({ size: contentLength, count: eTagCount });
                const httpStream = res.Body
    ...
    function findAllS3Objects(marker, prefix, cb) {
        ...
        function listObjects(cb) {
            ...
            self.s3Pend.go(function (pendCb) {
                ...
                s3Details.MaxKeys = 1000;
                self.s3.send(new ListObjectsCommand(s3Details), function (err, data) {
                    pendCb();
                    if (abort) return;
                    cb(err, data);
                });
...
Client.prototype.copyObject = function (_s3Params) {
    ...
    function doTheCopy(cb) {
        self.s3.send(new CopyObjectCommand(s3Params), cb);
    }
    return ee;
};
...
Client.prototype.downloadBuffer = function (s3Params) {
    ...
    function doTheDownload(cb) {
        ...
        const hashCheckPend = new Pend();
        request.then(resp => {
            const contentLength = parseInt(resp.ContentLength, 10);
            downloaderEE.progressTotal = contentLength;
            downloaderEE.progressAmount = 0;
            downloaderEE.emit('progress');
            downloaderEE.emit('httpHeaders', resp.ContentType);
            const eTag = cleanETag(resp.ETag);
            const eTagCount = resp.TagCount;

            const outStream = new StreamSink();
            const multipartETag = new MultipartETag({ size: contentLength, count: eTagCount });
            const httpStream = resp.Body;
    ...
    function doTheDownload(cb) {
        ...
        request.then(result => {
            if (result.httpStatusCode >= 300) {
                handleError(new Error('http status code ' + result.httpStatusCode));
                return;
            }
            downloadStream.emit('httpHeaders', result.ContentType);
            const httpStream = result.Body;
...
function syncDir(self, params, directionIsToS3) {
    ...
    function checkDoMoreWork() {
        ...
        function downloadS3Object() {
            ...
            if (getS3Params) {
                startDownload(fullPath, s3Object, haveS3Params);
            } else {
                startDownload();
            }
