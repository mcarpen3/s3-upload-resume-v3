...
Client.prototype.uploadFile = function(params) {
    var self = this;
    var uploaderEE = new EventEmitter();
    ...
    function startPuttingObject() {
        ...
        if (multipartUploadSize > MAX_PUTOBJECT_SIZE) {
            var err = new Error("File size exceeds maximum object size: " + uploaderEE.localFile);
            err.retryable = false;
            handleError(err);
            return;
        }
...
Client.prototype.downloadFile = function(params) {
    var self = this;
    var downloaderEE = new EventEmitter();
    downloaderEE.localFile = params.localFile;
    var s3Params = extend({}, params.s3Params);

    var dirPath = path.dirname(downloaderEE.localFile);
    ...
    function doTheDownload(cb) {
        ...
        if (headers['content-length'] == undefined) {
            var outStream = fs.createWriteStream(downloaderEE.localFile);
...
function syncDir(self, params, directionIsToS3) {
    ...
    function checkDoMoreWork() {
        ...
        var fullPath = path.join(ee.localDir, localFileStat.path);
        ee.deleteTotal += 1;
        ...
        s3ObjectCursor += 1;
        setImmediate(checkDoMoreWork);
        var fullPath = path.join(ee.localDir, toNativeSep(s3Object.key));
        ...
        function startDownload() {
            ...
            var downloaderEE = self.downloadFile(upDownFileParams);
            ...
            if (fatalError) return;
            var delta = downloaderEE.progressAmount - prevAmountDone;
        ...
        function startUpload() {
            ...
            upDownFileParams.s3Params.Key = fullKey;
            upDownFileParams.localFile = fullPath;
            var uploaderEE = self.uploadFile(upDownFileParams);
            var prevAmountDone = 0;
            ...
            if (fatalError) return;
            var amountDelta = uploaderEE.progressAmount - prevAmountDone;
            prevAmountDone = uploaderEE.progressAmount;
            ee.progressAmount += amountDelta;

            var totalDelta = uploaderEE.progressTotal - prevAmountTotal;
    ...
    function findAllFiles(cb) {
        var dirWithSlash = ensureSep(ee.localDir);
        ...
        var relPath = path.relative(ee.localDir, linkPath || dir);
        if (relPath === '') return;
        stat.path = relPath;
        ...
        if (fatalError) return walker.stop();
        var relPath = path.relative(ee.localDir, linkPath || file);
        stat.path = relPath;
        stat.s3Path = toUnixSep(relPath);
        ee.filesFound += 1;
